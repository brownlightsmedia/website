...
class App {
  ...
  onTouchUp(e) {
    ...
    const dragged = Math.abs(this.start - x) > 10;
    
    // Trigger onItemClick if it wasn't a drag
    if (!dragged && this.onItemClick && this.medias && this.medias[0]) {
      const width = this.medias[0].width;
      const itemIndex = Math.round(Math.abs(this.scroll.target) / width);
      const actualIndex = itemIndex % this.originalLength;
      
      // Determine if we clicked the center item by checking mouse position relative to center
      // Simplified: Just always select the centered item on click
      this.onItemClick(this.originalItems[actualIndex], actualIndex);
    }
    this.onCheck();
  }
  onWheel(e) {
